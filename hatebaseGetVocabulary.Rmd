...
1. Use the `content()` function from `httr` to extract the content from `r` and save it as a text object (`as = "text"`): `rText`
2. The `fromJSON()` function from the `jsonlite` package allows you to quickly parse the text in the `rText` object. 
...
1. Version number (`version`): already established in previous step 
2. Token (`token`): retrieved in previous step
3. Total number of pages in the Hatebase vocabulary dataset (`nPages`): the total number of pages changes with every version update, so you'll need to retrieve this before you can download the dataset
